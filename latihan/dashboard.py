...
def create_daily_orders_df(df: pd.DataFrame):
...
def create_sum_order_items_df(df: pd.DataFrame):
    sum_order_items_df = (
...
def create_bygender_df(df: pd.DataFrame):
...
def create_byage_df(df: pd.DataFrame):
...
all_df = pd.read_csv(f"{script_dir}/all_data.csv")
